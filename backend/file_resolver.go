...
func NewFileResolver(projectPath string, kb *KnowledgeBase) *FileResolver {
...
func (fr *FileResolver) findAlternatives(requestedFile string) []string {
	...
	case "composer-info.php":
		alternatives = append(alternatives, DependencyFileMapping["composer"]...)
	case "package-info.json":
		alternatives = append(alternatives, DependencyFileMapping["npm"]...)
	case "requirements-info.txt":
		alternatives = append(alternatives, DependencyFileMapping["python"]...)
	}
	...
	if strings.Contains(projectType, "go") {
		alternatives = append(alternatives, DependencyFileMapping["go"]...)
	}
...
func (fr *FileResolver) DiscoverProjectFiles() {
	...
	for depType, files := range DependencyFileMapping {
...
func (fr *FileResolver) GetAvailableAlternatives(fileType string) []string {
	...
	if files, exists := DependencyFileMapping[fileType]; exists {
		for _, file := range files {
			fullPath := filepath.Join(fr.projectPath, file)
			if fr.fileExists(fullPath) {
